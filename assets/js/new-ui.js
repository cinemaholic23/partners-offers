...
const heroStoriesData = [
  {
    image: "assets/images/tutu-hero.png",
    logos: [
      { image: "assets/images/tutu-logo.png", label: "Туту" }
    ],
    title: "Кэшбэк до\u00a020%\nна\u00a0отели в\u00a0Туту",
    subline: "до\u00a06 августа",
    cta: "Перейти к\u00a0покупкам"
  },
  {
    image: "assets/images/rest-hero.png?v=2",
    logos: [
      { image: "assets/images/rest-logo.png", label: "WRF" },
      { image: "assets/images/rappoport.png", label: "Rappoport" },
      { label: "+11", kind: "more" }
    ],
    title: "Привилегии в\u00a0ресторанах",
...
function createHeroIndicators(activeIndex) {
  const indicators = createElement("div", "hero-story__indicators");
  indicators.setAttribute("aria-hidden", "true");

  heroStoriesData.forEach((story, index) => {
    const classNames = ["hero-story__indicator"];

    if (index < activeIndex) {
      classNames.push("hero-story__indicator--complete");
    } else if (index === activeIndex) {
      classNames.push("hero-story__indicator--active");
    }

    const indicator = createElement("span", classNames.join(" "));

    if (index === activeIndex) {
      indicator.append(createElement("span", "hero-story__indicator-fill"));
    }
...
function createHeroStory(story, index) {
  ...
  const text = createElement("div", "hero-story__text");
  const actions = createElement("div", "hero-story__actions");
  const cta = createElement("button", "hero-story__cta", story.cta);
  ...
  story.logos.forEach((logo) => {
    ...
    const logoElement = createElement("span", className, logo.image ? "" : logo.label);
  ...
  text.append(createElement("h2", "hero-story__title", story.title));
...
const heroStories = document.querySelector("#heroStories");

if (heroStories) {
  initHeroStories(heroStories);
}
